...
export const addUserEmailTemplate = (
  ...
  const validationLink = `https://ws4mcufss9.execute-api.us-east-1.amazonaws.com/prod/event?tempRandToken=${tempRandToken}&discordId=${discordId}&email=${userEmail}&x=x`;
    ...
    <table id="table-root">
      <tr>
        ...
        ${
          channelsToJoin
            ? `<td class="email-line-item">
        Otherwise, please click the button below to validate your access to these exclusive channels:
      </td>
      <td id="purchased-product-display" class="email-line-item">
        <b>${channelsToJoin}</b>
      </td>`
            : `<td class="email-line-item">
          Otherwise, please click the button below to validate your additional purchases
          </td>
          `
        }
